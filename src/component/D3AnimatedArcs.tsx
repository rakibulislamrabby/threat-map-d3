...
const D3AnimatedArcs: React.FC = () => {
  ...
  const createAnimatedAttackArcs = useCallback((
    ...
    threats.attacks.forEach((attack, index) => {
      ...
      if (sourceCoord && targetCoord) {
        ...
        if (sourcePoint && targetPoint) {
          ...
          // Create glowing circle marker at attack origin point
          arcsGroup
            .append("circle")
            .attr("class", `origin-marker origin-${attack.severity}`)
            ...
            .on("mouseover", function(event) {
              // Make marker slightly larger and brighter on hover
              d3.select(this)
                .style("r", 6)
                .style("opacity", 1)
                .style("filter", `drop-shadow(0 0 10px ${severity.color})`);
            ...
            .on("mouseout", function() {
              // Reset marker to original size and opacity
              d3.select(this)
                .style("r", 4)
                .style("opacity", 0.9)
                .style("filter", `drop-shadow(0 0 6px ${severity.color})`);
          ...
          const createParticleAnimation = () => {
            ...
            (arcsGroup.node() as SVGElement & { __particleInterval?: NodeJS.Timeout }).__particleInterval = intervalId;
          };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      svg.selectAll('.origin-marker').remove();
    };
  }, [isClient, createAnimatedAttackArcs]);
  ...
  return (
    <div style={{
      ...
      {/* Live attack counter - only render on client */}
      {isClient && (
        <div style={{
          position: 'absolute',
          top: '20px',
